...
**`color` estandarizado, en dos pasadas el mismo día (2026-09-14):** a pedido explícito del usuario ("que hayan colores estandarizados y que el vendedor pueda elegir los colores que están"), tras encontrar en producción valores como `"cognac"` y `"Negro con mucho texto también"` (texto libre sin ninguna validación).

*Primera pasada — migración `standardize_prendas_color_palette`:* paleta cerrada de 17 colores (confirmada con el usuario en 2 rondas — pidió sacar "Cognac"/"Camel" de una primera propuesta y usar un set más parecido al de los marketplaces grandes de moda, no se pudo entrar a mercadolibre.com.ar a chequear el real porque el firewall de red de esa sesión bloquea la categoría "Shopping"): `Negro, Blanco, Gris, Beige, Marrón, Rojo, Naranja, Amarillo, Verde, Azul, Celeste, Violeta, Rosa, Bordo, Dorado, Plateado, Multicolor`. (1) UPDATE de normalización sobre las 28 prendas existentes — casing normalizado y consolidación semántica en 5 casos: `camel/crema/off-white → Beige`, `cognac/marrón oscuro → Marrón`, `navy → Azul`, `negro washed`/`"Negro con mucho texto también" → Negro`; (2) `ALTER TABLE ... ADD CONSTRAINT prendas_color_check CHECK (color IS NULL OR color IN (...))` — lista cerrada, ningún valor fuera de los 17 podía guardarse.

*Segunda pasada, un rato después, mismo día — migración `relax_prendas_color_check_allow_custom`:* el usuario pidió dos ajustes que revierten parcialmente la lista cerrada: (1) sacar "Multicolor" de la paleta (quedan 16); (2) agregar una opción **"Otro" con texto libre** en el selector, para casos que no entran en la paleta — un chip cerrado sin escape hatch no sirve si aparece un color legítimo que no está en la lista. Esto es **incompatible con un `CHECK` de lista cerrada** por diseño (el punto de "Otro" es guardar texto que no está en ningún lado), así que se reemplazó por un guardrail liviano: `ALTER TABLE prendas DROP CONSTRAINT prendas_color_check; ALTER TABLE prendas ADD CONSTRAINT prendas_color_length_check CHECK (color IS NULL OR char_length(btrim(color)) BETWEEN 1 AND 30)` — ya no fuerza pertenencia a la lista, solo que no esté vacío ni sea desmesuradamente largo (suficiente para bloquear el tipo de garbage que motivó todo esto). **La defensa "cierre absoluto contra texto libre" que ofrecía la primera pasada quedó descartada a propósito** — es un trade-off consciente pedido por el usuario, no un bug: por diseño de producto, un vendedor legítimo puede necesitar un color que no está en la paleta.
