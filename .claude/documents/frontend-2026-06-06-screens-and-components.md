...
- **Galería de imágenes (2026-09-14, antes era 1 sola foto):** `FlatList` horizontal `pagingEnabled` sobre `hooks/useGarmentImages.ts` (nuevo — lee `prenda_imagenes` ordenada por `sort_order`, cae a `prendas.image_url` si no hay filas), full-width, aspect ratio 1:1.1, con dots de paginación superpuestos abajo (`imageDots`/`imageDot`/`imageDotActive`, sin precedente de este patrón en el resto de la app) cuando hay más de 1 imagen. Tap en la imagen visible abre `ZoomableImage` con esa imagen (no siempre la portada — `galleryImages[activeImage]`, `activeImage` se actualiza en `onMomentumScrollEnd`). Antes cada prenda tenía como máximo 1 foto (`prendas.image_url`, sigue existiendo como la "portada" — ver `database-2026-06-06-schema-and-seed.md` → `prenda_imagenes`); ahora se pueden cargar hasta 5 desde `create-garment.tsx`. Verificado en browser real insertando una segunda imagen de prueba directo en la DB sobre una prenda real de Capas: swipe entre las dos fotos funciona, dots correctos (confirmado por DOM — 1 activo blanco 8×8, 1 inactivo translúcido 6×6), fila de prueba borrada después.
...
- **Imágenes — hasta 5, la primera es la portada (2026-09-14, antes era 1 sola):** a pedido explícito del usuario. Estado unificado `images: { uri, local }[]` — mezcla fotos recién elegidas (`local: true`, todavía sin subir) con las ya subidas (modo edición). Grilla de miniaturas 104×116 con badge "Portada" en la primera y botón `✕` de sacar por foto; tile "+ Agregar foto" al final mientras `images.length < 5`; contador debajo ("2/5 fotos — la primera es la portada"). `ImagePicker.launchImageLibraryAsync` con `allowsMultipleSelection`/`selectionLimit` (no compatible con `allowsEditing`, que se sacó — recortar no tiene sentido eligiendo varias a la vez). Al guardar: sube cada foto local (`lib/uploadImage.ts` ganó un parámetro `index` opcional para desambiguar el nombre de archivo cuando se suben varias en el mismo submit) y hace un reemplazo total de `prenda_imagenes` (`DELETE` + `INSERT` de la lista completa, directo contra Supabase — no pasa por la API, mismo patrón que `descontinuada`); `prendas.image_url` (el campo que sí va por la API) se setea siempre a la primera URL. Detalle completo, incluyendo cómo se resuelve el `garment_id` recién creado sin que la API lo devuelva, en `database-2026-06-06-schema-and-seed.md` → `prenda_imagenes`. Primera vez que la app sube un archivo desde el cliente (esto no cambió, solo ahora son varios): `expo-image-picker` (dependencia, plugin en `app.json`) + `lib/uploadImage.ts` sube el blob al bucket público `assets`, path `prendas/{marca-slug}/{prenda-slug}_{marca-slug}_{timestamp}[_{index}].{ext}` (mismo patrón de naming que ya documentaba `database-2026-06-06-schema-and-seed.md`, con timestamp en vez de "colección"). La extensión se deriva del `blob.type` del archivo elegido, no de la URI local — en web la URI es un `blob:` sin extensión real, parsearla daba `.jpg` para cualquier imagen. **Al menos 1 imagen es obligatoria**: se descubrió al probar (no al leer el schema) que `prendas.image_url` tiene `NOT NULL` en la DB.
- **Moderación básica por longitud + contador de caracteres (misma sesión, antes del multi-imagen):** a pedido explícito del usuario, todos los campos de texto libre tienen `maxLength` y un contador "X/N" debajo, alineado a la derecha (`styles.charCounter`) — Nombre (60), Descripción (500), Precio (9 dígitos, sin contador visible porque es numérico no texto), URL externa en modo redirect (300), y el input "Otro" de los `DropdownField` de Color/Estilo (30). Es moderación por longitud únicamente — no valida contenido ni lenguaje, solo evita texto desmedido antes de que llegue a la DB. Verificado en browser real: escribir un nombre de 132 caracteres se corta en 60; el contador incrementa en vivo con cada tecla.
