...
**Color estandarizado — chips en vez de texto libre (2026-09-14):** a pedido explícito del usuario, tras encontrar en producción colores como `"cognac"` y `"Negro con mucho texto también"` sin ninguna validación. El campo "Color" pasó de `TextInput` a una fila de chips single-select (mismo componente/estilo que Categoría y Talles), con toggle: tocar el color ya elegido lo deselecciona (sigue siendo opcional). Las opciones salen de la constante `constants/garmentColors.ts` (`GARMENT_COLORS`, 17 valores: Negro, Blanco, Gris, Beige, Marrón, Rojo, Naranja, Amarillo, Verde, Azul, Celeste, Violeta, Rosa, Bordo, Dorado, Plateado, Multicolor) — **tiene que mantenerse en sync a mano con el `CHECK constraint` `prendas_color_check` de la DB** (ver `database-2026-06-06-schema-and-seed.md`, no hay una tabla de lookup, son dos listas hardcodeadas independientes). El modo edición ya no necesitó ningún cambio aparte — como precarga `data.color` tal cual, y la DB ahora solo contiene valores de la paleta (migración `standardize_prendas_color_palette` corrió antes de este cambio de UI), el chip correcto queda preseleccionado solo. Verificado en browser real logueado como Capas: los 17 chips wrappean en 3 filas sin romper el layout, "Trench Camel" (antes `color:"camel"`, migrado a `"Beige"`) abre con el chip Beige activo, cambiar a Marrón y guardar hace un PATCH real que pasa el `CHECK constraint` sin error (confirmado en la DB, revertido a Beige después para no dejar el dato de prueba cambiado sin motivo).
...
- **Chip "Todo" + panel de filtros avanzados plegable en Prendas (2026-09-14):** a pedido explícito del usuario. Dos piezas:
  ...
  - **Color/Estilo son texto libre, no un enum** (`prendas.color`/`prendas.style`) — con datos reales inconsistentes en mayúsculas (`"Azul"` vs `"azul"`, y hasta un valor de prueba largo, `"Negro con mucho texto también"`, que quedó como chip real porque no hay motivo de producto para ocultarlo). Mismo criterio que `outfitTags`: se traen los valores reales que existen hoy (`SELECT color/style WHERE NOT NULL AND descontinuada=false`), deduplicados ignorando mayúsculas (`dedupeCaseInsensitive`, se queda con la primera variante de casing que aparece) — y el `textTransform: 'capitalize'` del chip además pareja visualmente casos como "marrón oscuro"/"Marrón Oscuro". El filtro en sí se aplica **client-side** después de traer los resultados (no con `.ilike()/.or()` de PostgREST): la tabla es chica (decenas de filas), y comparar en JS evita tener que armar/escapar a mano un string OR de PostgREST para case-insensitive-exacto. El `.limit()` de la query de prendas subió de 30 a 100 para no truncar antes de este filtro client-side (volumen real actual sigue siendo bajo, sin impacto de performance).
  - Verificado en browser real (desktop y mobile 375px): chip "Todo" trae las 4 categorías mezcladas; ícono cambia `filtro_negro.png`↔`filtro_rosa.png` al togglear (confirmado por `src` del `<img>`, no solo visualmente); seleccionar Negro+Gris en Color trae la unión de ambos (no la intersección); "Limpiar" resetea precio/orden/color/estilo sin tocar la categoría activa; en 375px de ancho los chips de Color/Estilo wrappean en varias líneas sin romper el layout ni desbordar horizontalmente. `tsc` sin errores nuevos (los de `constants/mockData.ts` son preexistentes, archivo sin uso real).
  - **Nota 2026-09-14:** tras estandarizar `prendas.color` (ver `Create Garment` más abajo y `database-2026-06-06-schema-and-seed.md`), este filtro no necesitó ningún cambio de código — como sigue trayendo valores reales de la tabla (mismo patrón que `outfitTags`, con `dedupeCaseInsensitive`), automáticamente pasó de mostrar 13 valores sucios (`cognac`, `Negro con mucho texto también`, etc.) a mostrar solo los colores estandarizados realmente en uso hoy (Azul, Beige, Blanco, Gris, Marrón, Negro — de los 17 posibles).
