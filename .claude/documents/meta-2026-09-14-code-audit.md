...
### ✅ B6. Manejo de errores en escrituras optimistas de `useCart` — RESUELTO (2026-09-15): revertir + toast

El usuario eligió la opción completa: si `updateQuantity`/`removeItem` fallan (RLS/red), el estado local vuelve al valor anterior (nunca queda mintiendo sobre lo que hay en la DB) y aparece un toast breve ("No se pudo actualizar la cantidad."/"No se pudo sacar el producto.", 1.8s, mismo patrón visual ya usado en `app/product/[id].tsx`). `hooks/useCart.ts` ahora chequea el `error` que devuelve cada llamada a Supabase (antes se ignoraba por completo), guarda una copia de `items` antes del update optimista para poder revertir, y loguea el error real a `error_logs` vía `logError()` (mismo mecanismo de B5). El hook expone `error`/`clearError`; `app/cart.tsx` (único consumidor de estos dos métodos) los conecta a un `useEffect` con `setTimeout` que auto-descarta el toast, replicando el patrón local ya existente en `product/[id].tsx` en vez de crear un componente `Toast` compartido nuevo (solo 2 lugares, no ameritaba la abstracción). **No se tocó `addItem`** — ya tenía su propio guard con `try/finally` y no estaba en el alcance de este ítem (solo mencionaba `updateQuantity`/`removeItem`). `tsc --noEmit` limpio.

### ✅ B7. Dependencias de `package.json` sin uso directo confirmado — RESUELTO (2026-09-15)

El usuario eligió "intentarlo con cuidado". Sacadas de `dependencies`/`devDependencies`: `nativewind`, `tailwindcss` (sin `tailwind.config` en ningún lado), `expo-constants`, `react-native-gesture-handler`, `react-native-reanimated`, `react-native-worklets`, `react-native-is-edge-to-edge`, `@types/react-native` (deprecado desde RN 0.71+). `babel-preset-expo` se movió de `dependencies` a `devDependencies` (sí se usa, en `babel.config.js`, pero en la categoría equivocada).

**Un hallazgo real a mitad de camino:** `expo-linking` también estaba en la lista original para sacar (el código de la app usa `Linking` de `react-native` core, no ese paquete) — pero al reinstalar con un `package-lock.json` limpio, el bundle de web **rompió de verdad**: `expo-router` hace un `require("expo-linking")` directo dentro de su propio `Unmatched.js` (pantalla de ruta no encontrada), así que aunque ningún archivo de `opa-mobile` lo importe, sí hace falta en el árbol de dependencias reales. Se restauró `expo-linking` en `dependencies` antes de seguir — es la razón exacta por la que el propio audit marcó esto como "requiere probar la app entera, no solo `tsc`" en vez de asumir que un `grep` sin resultados alcanza.

**Verificación real:** `npm install --legacy-peer-deps` con lockfile limpio, `tsc --noEmit` limpio, y la app probada de punta a punta en un browser real contra Supabase (no el preview del harness — el `preview_start` del Browser pane falló de forma intermitente con `TypeError: fetch failed` en varios intentos, algo ya visto en sesiones anteriores con este entorno y no relacionado a este cambio; se verificó levantando `expo start --web` a mano vía PowerShell en un puerto alternativo): Home con datos reales (outfits, últimas prendas), outfit scroll TikTok-style (paginado por swipe/scroll, chips conectados a cada prenda, precio total), detalle de prenda (selector de talle, stepper, Q&A) y el visor de zoom de `ZoomableImage` (abrir con doble-tap + cerrar con el botón ✕, ambos con clicks simulados) — todo funcionando sin ninguna de las 8 dependencias sacadas. Sin errores de consola ni de bundling (`Unable to resolve` para ningún módulo).

### ✅ B8. Tamaños de touch target < 44×44 e iconografía "volver" inconsistente — RESUELTO (2026-09-15)

El usuario aprobó ambas partes.

- **Ícono "volver":** las 6 pantallas que seguían con el glifo de texto `←` (`app/outfit/[id].tsx` — 3 apariciones, `app/settings.tsx`, `app/measurements.tsx`, `app/switch-account.tsx`, `app/user-outfits.tsx`, `app/saved-outfits.tsx`) ahora usan `flecha.png` desde `STORAGE_BASE_URL`, igual que las 8 pantallas que ya lo hacían. Dos variantes según el fondo: header blanco (`outfit/[id]`, `settings`, `measurements`, `switch-account`) usa la imagen tal cual (negra); botón flotante sobre foto (`user-outfits`, `saved-outfits`) agrega `tintColor={colors.blanco}` para que siga siendo visible sobre las fotos oscuras, igual que antes con el glifo blanco.
- **Touch targets del outfit scroll:** `components/outfit/OutfitScrollItem.tsx` — `actionBtn` (contenedor de like/save/compartir) pasó de 34×34 a 44×44; los íconos internos (`actionIcon` texto ♥/★, `shareIcon` imagen) se agrandaron proporcionalmente (30→32, 26→28) para no quedar chicos dentro del botón más grande.

**Verificado:** `tsc --noEmit` limpio. Probado en un browser real (mismo mecanismo que B7 — server manual vía PowerShell, el `preview_start` del harness seguía fallando intermitente): outfit scroll con los 3 botones visiblemente más grandes y bien espaciados (confirmado también por DOM: 44×44 exacto), sin regresión visual. No se re-verificaron las 6 pantallas del ícono de volver una por una en el browser (cambio mecánico de bajo riesgo, mismo patrón ya probado en las 8 pantallas existentes) — si algo se ve mal, es la primera cosa a mirar.
...
- **B0–B8: todos resueltos** (2026-09-14/15). La auditoría de `meta-2026-09-14-code-audit.md` está completa — no queda ningún ítem pendiente de decisión en este documento.
